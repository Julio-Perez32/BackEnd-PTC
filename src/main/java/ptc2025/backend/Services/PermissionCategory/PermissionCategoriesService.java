...
public class PermissionCategoriesService {
    ...
    public PermissionCategoriesDTO insertPermission(PermissionCategoriesDTO categories) {
        if (categories == null || categories.getCategoryName() == null || categories.getCategoryName().isEmpty()) {
            throw new ExceptionBadRequest("El permiso debe de contener un nombre.");
        }
        try {
            ...
            throw new RuntimeException("Error al registrar la categoria.");
        }
    }

    public PermissionCategoriesDTO updatePermission(String id, PermissionCategoriesDTO json) {
        PermissionCategoriesEntity existsPermission = repo.findById(id)
                .orElseThrow(() -> new ExceptionNoSuchElement("Permiso no encontrado con ID: " + id));
    ...
    public boolean deletePermission(String id) {
        try {
            ...
            throw new ExceptionNoSuchElement("No se encontró permiso con ID: " + id + " para eliminar.");
        }
    }
}
